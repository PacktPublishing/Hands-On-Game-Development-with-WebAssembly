...
"void main() {  \n"
        "gl_Position = vec4(u_translate, 0.0, 0.0) + a_position;   \n"
...
//    "normal = normalize(normal * 2.0 - 1.0);    \n"
    ...
    "gl_FragColor = texture2D(u_texture, v_texcoord) * diffuse;   \n"
"}  \n"
};
*/
...
const GLchar* fragment_shader_code[] = {
      ...
      "const vec3 view_pos = vec3(400, 300, 50);   \n"
      "const vec4 light_color = vec4( 0.8, 1.0, 0.8, 0.0); \n"
      ...
      "void main() {    \n"                             
        ...
        "vec3 norm = normalize(texture2D(u_normal, v_texcoord).rgb); \n"
        "vec3 light_dir = normalize(gl_FragCoord.xyz - light_pos);  \n"
        ...
        /*
        "vec3 light = gl_FragCoord.xyz - vec3(400, 300, -100);   \n"
        "float dist = length(light);    \n"
        "float norm_dot_light = dot( norm, normalize( light ) );  \n"
        "gl_FragColor = (tex * ambient) + clamp((tex * ( (norm_dot_light*50000.0)/(dist*dist) ) ), -ambient, ambient);   \n"
        */
      "}    \n"
};
...
const GLchar* fragment_shader_code[] = {
      ...
      "void main() {    \n"
        "vec4 tex = texture2D(u_texture, v_texcoord);   \n"
        "vec3 norm = texture2D(u_normal, v_texcoord).rgb * texture2D(u_normal, v_texcoord).aaa; \n"
        "vec3 light = gl_FragCoord.xyz - vec3(400, 300, -100);   \n"
        "float dist = length(light);    \n"
        "float norm_dot_light = dot( norm, normalize( light ) );  \n"
        "gl_FragColor = (tex * ambient) + clamp((tex * ( (norm_dot_light*50000.0)/(dist*dist) ) ), -ambient, ambient);   \n"
